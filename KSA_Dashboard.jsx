...
const C = {
  accent: "#E8563A", accentL: "#FFF0ED", bg: "#F5F2EE",
  ...
  return (
    <div style={{ display: "flex", height: "100vh", alignItems: "center", justifyContent: "center", background: C.bg }}>
      <div style={{ background: C.white, borderRadius: 14, padding: 36, width: 440, boxShadow: "0 4px 24px rgba(0,0,0,.08)", border: `1px solid ${C.border}` }}>
        <div style={{ fontSize: 22, fontWeight: 700, letterSpacing: "-.4px", marginBottom: 4 }}>Waffarha KSA 🇸🇦</div>
        <div style={{ fontSize: 13, color: C.muted, marginBottom: 24 }}>Merchant Intelligence Dashboard</div>
    ...
    <div style={{ display: "flex", height: "100vh", alignItems: "center", justifyContent: "center", background: C.bg }}>
      <div style={{ background: C.white, borderRadius: 14, padding: 36, width: 360, boxShadow: "0 4px 24px rgba(0,0,0,.08)", border: `1px solid ${C.border}` }}>
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          <div style={{ width: 52, height: 52, borderRadius: 14, background: C.accentL, display: "inline-flex", alignItems: "center", justifyContent: "center", marginBottom: 14 }}>
...
/* ─── PROFILER TAB ───────────────────────────────────────────── */
function ProfilerTab({ merchants, anonKey }) {
  ...
  const pitchStyle = {
    "The Capacity Filler": { background: "#FFF0ED", color: "#C2410C" },
    "The Menu Hero": { background: "#EFF6FF", color: "#1D4ED8" },
    "The Traffic Controller": { background: "#FEF3C7", color: "#92400E" },
    "The Visibility Boost": { background: "#F0FDF4", color: "#15803D" },
  };
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          messages: [{
            role: "user", content: `You are a BD analyst for a Saudi Arabia deals platform. Analyze this merchant and return ONLY valid JSON (no markdown, no backticks):

Merchant: ${selected.Merchant}
Location: ${selected.Mall}, ${selected.City}
Rating: ${selected.Rating}/5 | Reviews: ${selected.Reviews.toLocaleString()} | Price: ${selected.AvgPrice || "Unknown"} | Branches: ${selected.Branches}
...
{"sentiment_score":<0-100>,"top_praise":"<8 words Arabic>","top_complaint":"<8 words Arabic or null>","menu_heroes":["<dish>"],"price_sensitivity":"<Value for Money|Overpriced|Cheap>","delivery_mentions":<true|false>,"bd_priority_score":<0-100>,"recommended_pitch_angle":"<The Capacity Filler|The Menu Hero|The Traffic Controller|The Visibility Boost>","pitch_script":"<2 Arabic sentences>"}`
    ...
    } catch { setErr("Analysis failed — merchant may have no reviews."); }
    setLoading(false);
  }
    ...
    <div>
      ...
      <div style={{ position: "relative", marginBottom: 16 }}>
        ...
        {showList && filtered.length > 0 && (
          <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: C.white, border: `1px solid ${C.border}`, borderRadius: 8, zIndex: 10, maxHeight: 230, overflowY: "auto", marginTop: 3, boxShadow: "0 4px 16px rgba(0,0,0,.08)" }}>
            {filtered.map((m, i) => (
              <div key={i} onClick={() => { setSelected(m); setQuery(m.Merchant); setSemanticNames(null); setShowList(false); setResult(null); }}
      ...
      {selected ? (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1.2fr", gap: 16 }}>
          ...
          <Card>
            <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 12 }}>AI Business Intelligence</div>
            <button onClick={analyze} disabled={loading}
              style={{ width: "100%", padding: 11, background: loading ? C.border : C.accent, color: loading ? C.muted : "#fff", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 500, cursor: loading ? "not-allowed" : "pointer", marginBottom: 14 }}>
              {loading ? "Analyzing reviews…" : "Analyze with AI →"}
            </button>
            {loading && <div style={{ display: "flex", justifyContent: "center", gap: 5, padding: 16 }}>
              {[0, 150, 300].map(d => <div key={d} style={{ width: 7, height: 7, borderRadius: "50%", background: C.accent, animation: "bounce .7s infinite alternate", animationDelay: `${d}ms` }} />)}
            </div>}
            {err && <div style={{ color: C.accent, fontSize: 12, textAlign: "center", padding: 10 }}>{err}</div>}
            {result && (
              <div>
                <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
                  <div style={{ width: 56, height: 56, borderRadius: "50%", flexShrink: 0, background: `${scoreColor(result.bd_priority_score)}22`, color: scoreColor(result.bd_priority_score), display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 800 }}>
                    {result.bd_priority_score}
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 10, color: C.muted, textTransform: "uppercase", letterSpacing: .5 }}>BD Priority Score</div>
                    <div style={{ fontSize: 17, fontWeight: 700, color: scoreColor(result.bd_priority_score) }}>
                      {result.bd_priority_score >= 80 ? "Hot Lead 🔥" : result.bd_priority_score >= 60 ? "Warm Lead" : "Cold Lead"}
                    </div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: 10, color: C.muted }}>Sentiment</div>
                    <div style={{ fontSize: 20, fontWeight: 800, color: scoreColor(result.sentiment_score) }}>{result.sentiment_score}%</div>
                  </div>
                </div>
                {result.recommended_pitch_angle && (
                  <div style={{ display: "inline-flex", padding: "5px 10px", borderRadius: 6, fontSize: 11, fontWeight: 500, marginBottom: 10, ...(pitchStyle[result.recommended_pitch_angle] || { background: "#F4F2EE", color: C.sub }) }}>
                    {result.recommended_pitch_angle}
                  </div>
                )}
                {[["Top Praise", result.top_praise, "#F9F8F7"], ["Top Complaint", result.top_complaint, "#FFF0ED"]].map(([l, v, bg]) => v ? (
                  <div key={l} style={{ marginBottom: 8 }}>
                    <div style={{ fontSize: 10, color: C.muted, textTransform: "uppercase", letterSpacing: .5, marginBottom: 3 }}>{l}</div>
                ...
                <div style={{ marginBottom: 8 }}>
                  <div style={{ fontSize: 10, color: C.muted, textTransform: "uppercase", letterSpacing: .5, marginBottom: 5 }}>BD Pitch Script</div>
                  <div style={{ background: "#FFF8F0", border: `1px solid #FEE2D5`, borderRadius: 7, padding: 10, fontSize: 12, color: "#7C2D12", lineHeight: 1.8, direction: "rtl", textAlign: "right" }}>
                    {result.pitch_script}
                  </div>
                </div>
                <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
                  {[result.price_sensitivity, result.delivery_mentions ? "Delivery Mentioned" : "No Delivery"].map((t, i) => (
                    <span key={i} style={{ fontSize: 10, padding: "3px 8px", borderRadius: 4, background: i === 0 ? "#F0F9FF" : "#F4F2EE", color: i === 0 ? "#0369A1" : C.sub, fontWeight: 500 }}>{t}</span>
                  ))}
                </div>
              </div>
            )}
            {!result && !loading && (
              <div style={{ textAlign: "center", padding: 20, color: C.muted, fontSize: 12, lineHeight: 2 }}>
                Click Analyze to get:<br />
                • Sentiment Score • Top Praise & Complaint<br />
                • Menu Heroes • BD Priority Score<br />
                • Arabic Pitch Script
              </div>
            )}
          </Card>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 14, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <span style={{ marginLeft: "auto", fontSize: 12, color: C.muted }}>{filteredMerchants.length} matches ({rows.length} shown)</span>
      </div>
              ...
              {rows.map((m, i) => {
                ...
                return (
                  <tr key={i} style={{ borderBottom: `1px solid #F4F2EE` }}>
                    <td style={{ padding: "9px 12px", fontWeight: 500, maxWidth: 160, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", direction: "rtl" }}>{m.Merchant}</td>
                    ...
                    <td style={{ padding: "9px 12px", color: C.muted, fontSize: 11, maxWidth: 100, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.OpeningHours || "—"}</td>
    ...
    <div style={{ display: "flex", height: "100vh", fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif", background: C.bg, color: C.text, overflow: "hidden" }}>
      ...
      <div style={{ width: 216, background: C.white, borderRight: `1px solid ${C.border}`, display: "flex", flexDirection: "column", flexShrink: 0 }}>
        <div style={{ padding: "20px 16px 14px", borderBottom: `1px solid ${C.border}` }}>
          <div style={{ fontSize: 14, fontWeight: 700, letterSpacing: "-.3px" }}>Waffarha KSA</div>
          <div style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>Merchant Intelligence</div>
        </div>
      ...
      <main style={{ flex: 1, overflowY: "auto", padding: 24 }}>
        ...
        {tab === "profiler" && <ProfilerTab merchants={unifiedMerchants} anonKey={anonKey} />}
        {tab === "pipeline" && <PipelineTab merchants={unifiedMerchants} />}
      </main>
    </div>
  );
}