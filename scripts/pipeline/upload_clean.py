...
def upload():
    data = json.load(open('output/clean_master.json', 'r', encoding='utf-8'))['enriched']
    ...
    for city, merchants in city_data.items():
        ...
        for m in merchants:
            recs.append({
                "merchant_name": m.get("merchant"), "mall": m.get("mall"), "city": m.get("city"),
                "priority": m.get("priority"), "category": m.get("category"), "rating": m.get("rating"),
                "reviews_count": m.get("reviews"), "avg_price": m.get("avg_price"), "branches_ksa": m.get("branches"),
                "phone": m.get("phone"), "website": m.get("website"), "opening_hours": m.get("opening_hours"),
                "lat": m.get("lat"), "lng": m.get("lng"), "top_reviews": m.get("top_reviews")
            })
